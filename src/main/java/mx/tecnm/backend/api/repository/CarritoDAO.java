...
public class CarritoDAO {
    ...
    public List<Detalles_carrito> obtenerCarrito(int usuarios_id) {
        String sql = """
        ...
        return jdbcClient.sql(sql)
                .param(usuarios_id)
                .query(new Detalles_carritoRM()) // Llamamos al mapper externo
                .list();
    }

    // 2. AGREGAR PRODUCTO (Insertar o Incrementar)
    public void agregarProducto(int usuarios_id, int producto_id, int cantidad) {
        ...
        Integer count = jdbcClient.sql(sqlCheck).param(usuarios_id).param(producto_id).query(Integer.class).single();
        ...
        if (count > 0) {
            ...
            jdbcClient.sql(sqlUpdate).param(cantidad).param(usuarios_id).param(producto_id).update();
        } else {
            // C) NO EXISTE -> Buscamos precio actual e insertamos
            String sqlPrecio = "SELECT precio FROM productos WHERE id = ?";
            var precio = jdbcClient.sql(sqlPrecio).param(producto_id).query(java.math.BigDecimal.class).single();

            String sqlInsert = "INSERT INTO detalles_carrito (usuarios_id, productos_id, cantidad, precio) VALUES (?, ?, ?, ?)";
            jdbcClient.sql(sqlInsert).param(usuarios_id).param(producto_id).param(cantidad).param(precio).update();
        }
    }

    // 3. ELIMINAR UN ITEM (Borrado real)
// 3. ELIMINAR UN ITEM (Lógica Inteligente: Disminuir o Borrar)
public boolean eliminarProducto(int usuarios_id, int producto_id) {
    ...
    if (cantidadActual > 1) {
        ...
        jdbcClient.sql(sqlUpdate).param(usuarios_id).param(producto_id).update();
        ...
        jdbcClient.sql(sqlDelete).param(usuarios_id).param(producto_id).update();
        return true; // Se eliminó con éxito
    }
}

    // 4. LIMPIAR CARRITO COMPLETO
    public void limpiarCarrito(int usuarios_id) {
        String sql = "DELETE FROM detalles_carrito WHERE usuarios_id = ?";
        jdbcClient.sql(sql).param(usuarios_id).update();
    }
}